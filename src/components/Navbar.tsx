...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'recovery', label: 'Recovery Mode', shortLabel: 'Recovery', icon: HeartPulse, isRecovery: true },
    { id: 'coach', label: 'Coach & Triage', shortLabel: 'Coach', icon: Layers },
        ...
        <div className="flex items-center justify-between h-16 gap-2">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                {recoveryState.isActive && (
                  <span className="bg-emerald-500/15 border border-emerald-400/40 text-emerald-600 dark:text-emerald-400 text-[10px] font-extrabold px-2 py-0.5 rounded-full flex items-center gap-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 xl:px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-150 whitespace-nowrap shrink-0 ${
                    isActive
                      ...
                      : isRec && recoveryState.isActive
                      ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 font-bold hover:bg-emerald-500/20'
                      : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-white/40 dark:hover:bg-white/10'
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 xl:w-4 xl:h-4 ${isActive ? 'text-white' : isRec ? 'text-emerald-500' : 'text-slate-400'}`} />
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-md shadow-emerald-600/20 cursor-pointer"
              title="Quick Health & Strain Check-In"
            >
              <HeartPulse className="w-3.5 h-3.5 animate-pulse" />
              <span className="hidden sm:inline">Energy Check-In</span>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap border ${
                  isActive
                    ...
                    : isRec && recoveryState.isActive
                    ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border-emerald-400/40 font-bold'
