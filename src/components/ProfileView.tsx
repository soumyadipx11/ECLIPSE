        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#121418] border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Are you sure you want to delete your profile <strong className="text-slate-900 dark:text-white">({user?.email})</strong>? This will permanently wipe your profile settings, pre-existing health conditions, health goals, and all linked lab report documents.
            </p>

            {deleteError && (
              <div className="p-3 bg-rose-50 dark:bg-rose-950/50 border border-rose-200 dark:border-rose-900/50 rounded-xl text-rose-600 dark:text-rose-400 text-xs flex items-start gap-2 animate-in fade-in">
            ...
            {isPasswordUser && (
              <div className="space-y-1.5 pt-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-[#181a20] border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-rose-500"
                />
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={deletingProfile || (isPasswordUser && !deleteConfirmPassword.trim())}
                className="px-4 py-2.5 bg-rose-600 hover:bg-rose-500 text-white font-semibold rounded-xl text-xs transition-all shadow-md shadow-rose-600/20 disabled:opacity-50 flex items-center gap-2 cursor-pointer"
              >
                {deletingProfile ? (
                  <>
                    <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
